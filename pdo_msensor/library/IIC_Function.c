 ...
 * 		Gain			*	X	+	Offset
 * [1,0;	3,2;	5,4	 ]	[X]		[19,18]
 * [7,6;	9,8;	11,10]	[Y]	+	[21,20]
 * [13,12;	15,14;	17,16]	[Z]		[23,22]
 */ 
/**
 * M-sensor
 */ 
extern idata short GainMatrix_M[12];
/**
 * Gyro
 */
extern idata short GainMatrix_G[12];
...
void I2C_M_Data(unsigned char device, unsigned char address, bit ReadHighResolution)
{
	...
	*/
	...
	CalibrationCaculate(&X_Data, &Y_Data, &Z_Data, &GainMatrix_M);
						 ...
						 short *GainMatrix)
{
	...
	TransformedSignedTwoBytesIntX = (SignedTwoBytesIntX * GainMatrix[0] +
									 SignedTwoBytesIntY * GainMatrix[1] + SignedTwoBytesIntZ * GainMatrix[2] + GainMatrix[3]) /
									100;

	TransformedSignedTwoBytesIntY = (SignedTwoBytesIntX * GainMatrix[4] +
									 SignedTwoBytesIntY * GainMatrix[5] + SignedTwoBytesIntZ * GainMatrix[6] + GainMatrix[7]) /
									100;

	TransformedSignedTwoBytesIntZ = (SignedTwoBytesIntX * GainMatrix[8] +
									 SignedTwoBytesIntY * GainMatrix[9] + SignedTwoBytesIntZ * GainMatrix[10] + GainMatrix[11]) /
									100;

	Xorigin[1] = (char)(TransformedSignedTwoBytesIntX);	  // Low byte
	Xorigin[0] = (char)(TransformedSignedTwoBytesIntX >> 8); // High byte
	Yorigin[1] = (char)(TransformedSignedTwoBytesIntY);	  // Low byte
	Yorigin[0] = (char)(TransformedSignedTwoBytesIntY >> 8); // High byte
	Zorigin[1] = (char)(TransformedSignedTwoBytesIntZ);	  // Low byte
	Zorigin[0] = (char)(TransformedSignedTwoBytesIntZ >> 8); // High byte
}
